from utils import *
import os
os.environ['CUDA_VISIBLE_DEVICE']='3,4'
os.environ['LLAMA2_PATH'] = '/home/wangzeqing/kn/Tender-main/Llama'
...
for SIZE in ['7b']:
    for SEQLEN in [256,32]:
        for DATASET in ["wikitext2"]:
            cmd = "CUDA_VISIBLE_DEVICES=0 python llama.py "
            cmd += "--model %s/Llama-2-%s-hf "%(llama_2_dir, SIZE)
            cmd += "--eval_dataset %s "%(DATASET)
            cmd += "--seq_len %d "%(SEQLEN)
            cmd += "--eval_samples %d "%(EVAL_SAMPLES)
            print(cmd)
